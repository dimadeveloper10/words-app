...
This is an educational project for learning Node.js, PostgreSQL, and React. When
implementing a requested change, do not provide only the finished code:

- Explain what is being changed, what each important part is responsible for, and
  how the pieces work together.
- Explain why the chosen approach fits this project. Mention meaningful alternatives
  and trade-offs when they help understanding, without overwhelming the answer.
- Connect new code to the relevant Node.js/NestJS, PostgreSQL/TypeORM, or React
  concepts, especially when introducing unfamiliar syntax, abstractions, libraries,
  configuration, database relations, migrations, state, or data flow.
- When giving terminal commands, explain what they do and call out important flags or
  side effects, particularly for migrations, dependency installation, and destructive
  operations.
- After implementation, summarize the resulting request/data flow and the checks or
  tests that were run. Keep explanations proportional to the task; avoid narrating
  obvious syntax line by line.

## Commit naming

Follow the existing repository history for commit messages:

- For work tied to a tracked task, use
  `[WA-<number>] <Component>: <concise English summary>`.
- Use `Admin:` for changes to the React admin panel and `Api:` for changes to the
  NestJS backend, for example `[WA-14] Admin: edit word dialog` or
  `[WA-14] Api: edit word dialog`.
- Do not use Conventional Commit-style prefixes such as `feat(words):`.
- Keep the summary focused on one change, use imperative wording where practical,
  and do not end it with a period.
- Use only a task number provided by the user or available in the task/branch
  context. Never invent a `WA` number.
- The `[WA-<number>]` prefix is mandatory. If the task number is unavailable, do not
  create the commit; ask the user for it first. Automatically generated merge commit
  messages are exempt.
